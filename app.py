...
st.markdown(
    ...
    .screener-row {
        display: grid;
        grid-template-columns: 38px 1fr 72px 56px 72px;
        align-items: center;
        gap: 0.5rem;
        padding: 0.85rem 1rem;
        border-bottom: 1px solid var(--border);
    ...
    .screener-row.data:hover { background: rgba(244,162,97,0.04); }
    .screener-row .rank {
    ...
    .screener-row .num.dim {
        color: var(--text-dim);
        font-weight: 500;
    }
    .screener-row .delta-pct {
        font-family: 'Geist Mono', monospace;
        font-size: 0.82rem;
        font-weight: 600;
        text-align: right;
    }
    .screener-row .delta-pct.neg { color: var(--bear); }
    .screener-row .delta-pct.pos { color: var(--bull); }
    .screener-row .rsi-val {
        font-family: 'Geist Mono', monospace;
        font-size: 0.9rem;
        font-weight: 700;
        color: var(--gold);
    ...
    .screener-meta .count { color: var(--accent); font-weight: 600; }

    /* Refresh button restyle */
    ...
    /* Mobile: collapse screener columns */
    @media (max-width: 520px) {
        .screener-row {
            grid-template-columns: 30px 1fr 56px 48px;
            gap: 0.4rem;
            padding: 0.7rem 0.8rem;
        }
        .screener-row .dist-col { display: none; }
    }
...
# =============================================================================
# 2b. SCREENER — batch scan of an universe
# =============================================================================

@st.cache_data(ttl=1800, show_spinner=False)
def scan_universe(tickers: tuple[str, ...], universe_label: str) -> pd.DataFrame:
    """
    Batch-scan a list of tickers for the confluence signal, based on last
    closed daily bar. Returns a DataFrame sorted by signal strength.

    Strategy:
    - Single yf.download call with threads=True for max parallelism
    - Compute indicators per-ticker on the grouped result
    - Filter: is_liquid=True AND price<=lower_band AND rsi<=30
    - Sort descending by "depth": how deep into both oversold + sub-band

    The tuple(tickers) signature is intentional — lists aren't hashable so
    Streamlit's cache would break without coercing to tuple.
    """
    if not tickers:
        return pd.DataFrame()

    try:
        # group_by='ticker' returns a MultiIndex df with one block per symbol
        data = yf.download(
            list(tickers),
            period="1y",
            interval="1d",
    ...
    for tk in tickers:
        try:
            # Handle both single-ticker (flat cols) and multi-ticker (MultiIndex) cases
            if isinstance(data.columns, pd.MultiIndex):
                if tk not in data.columns.get_level_values(0):
                    continue
                sub = data[tk].dropna(subset=["Close"]).copy()
            else:
                sub = data.dropna(subset=["Close"]).copy()
            ...
            lower, middle, upper = compute_bollinger(close, 20, 2.0)
            lower_v, middle_v = float(lower.iloc[-1]), float(middle.iloc[-1])
            price = float(close.iloc[-1])
            prev = float(close.iloc[-2]) if len(close) >= 2 else price
            avg_vol = float(sub["Volume"].tail(90).mean())

            # Skip if essential values are NaN
            if any(pd.isna(x) for x in [rsi, lower_v, middle_v, price, avg_vol]):
                continue

            is_liquid = avg_vol > 300_000
            confluence = (price <= lower_v) and (rsi <= 30)

            if not (is_liquid and confluence):
                continue

            # Signal strength — how strong is the oversold + sub-band combo
            # depth_pct: how far below BB Lower (positive = below)
            depth_pct = (lower_v - price) / lower_v * 100
            # rsi_intensity: how far below 30 (positive = deeper oversold)
            rsi_intensity = max(0.0, 30.0 - rsi)
            # Composite score — equal weight, normalized
            strength = depth_pct + rsi_intensity

            # Upside to mean-reversion target
            upside_pct = (middle_v - price) / price * 100
            ...
            rows.append({
                ...
                "rsi": rsi,
                "delta_pct": delta_pct,
                "dist_pct": (price - lower_v) / lower_v * 100,  # negative = below
    ...
    df = pd.DataFrame(rows).sort_values("strength", ascending=False).head(10)
...
def resolve_name(ticker: str) -> str:
    """Fast lookup for display name — cached aggressively."""
    try:
...
# =============================================================================
# SCREENER SECTION — batch scan of Milan + Nasdaq
# =============================================================================
st.markdown(
    '<div class="section-head"><span class="num">00</span>'
    '<span class="title">Screener · Titoli in Confluenza</span></div>',
    unsafe_allow_html=True,
)

st.markdown(
    """
    <div style="font-family: 'Fraunces', serif; font-style: italic; color: var(--text-dim);
                font-size: 0.95rem; line-height: 1.55; margin-bottom: 1rem;">
        Scansione in tempo reale di Milano e Nasdaq. Mostra fino a 10 titoli
        che soddisfano <strong>contemporaneamente</strong> prezzo sotto BB Lower
        e RSI ≤ 30, ordinati per intensità del segnale.
    </div>
    """,
    unsafe_allow_html=True,
)
...
# Force cache bust if user clicks refresh
if refresh:
    scan_universe.clear()

with st.spinner(f"Scansione in corso su {len(UNIVERSES[universe_choice])} titoli…"):
    screener_df = scan_universe(
        tuple(UNIVERSES[universe_choice]),
        universe_choice,
    )
...
if screener_df.empty:
    st.markdown(
        ...
        <div class="screener-empty">
            <div class="stamp">— Nessun Match</div>
            <div class="msg">
                Al momento nessun titolo in <strong>{universe_choice}</strong>
                soddisfa contemporaneamente le condizioni di RSI ≤ 30 e prezzo
                sotto la banda inferiore.<br><br>
                Questo è <em>normale</em>: in un mercato in salute, la confluenza
                è un evento relativamente raro. Riprova dopo una giornata di forte
                correzione.
            </div>
        </div>
        """,
    ...
    # Build table HTML
    rows_html = [
        '<div class="screener-row header">'
        '<div class="rank">#</div>'
        '<div class="name-col">TITOLO</div>'
        '<div class="num">PREZZO</div>'
        '<div class="num">RSI</div>'
        '<div class="num">UPSIDE</div>'
        '</div>'
    ]
    for i, row in screener_df.iterrows():
        tk = row["ticker"]
        name = resolve_name(tk)
        # Truncate long names
        name_display = name if len(name) <= 28 else name[:26] + "…"
        delta_cls = "neg" if row["delta_pct"] < 0 else "pos"
        delta_sign = "" if row["delta_pct"] < 0 else "+"
        upside_sign = "+" if row["upside_pct"] >= 0 else ""

        rows_html.append(
            f"""
            <div class="screener-row data">
                <div class="rank"><span class="hash">№</span>{i+1}</div>
                <div class="name-col">
                    <div class="tk">{tk}</div>
                    <div class="nm">{name_display}</div>
                </div>
                <div>
                    <div class="num">{row['price']:.2f}</div>
                    <div class="delta-pct {delta_cls}">{delta_sign}{row['delta_pct']:.2f}%</div>
                </div>
                <div class="rsi-val">{row['rsi']:.0f}</div>
                <div class="num" style="color: var(--bull);">{upside_sign}{row['upside_pct']:.1f}%</div>
            </div>
            """
        )

    st.markdown(
        f'<div class="screener-wrap">{"".join(rows_html)}</div>',
        unsafe_allow_html=True,
    )
    ...
    st.markdown(
        ...
        <div class="screener-meta">
            <span><span class="count">{len(screener_df)}</span> titoli in confluenza
            su {len(UNIVERSES[universe_choice])} scansionati</span>
            <span>Upside = target BB Mid · Cache 30min</span>
        </div>
        """,
        unsafe_allow_html=True,
    )
    ...
    st.caption(
        "💡  Clicca il tasto **↻ Aggiorna** per forzare una nuova scansione. "
        "I dati si basano sull'ultima chiusura di borsa disponibile."
    )

# ---- Input ----
st.markdown(
    '<div class="section-head"><span class="num">01</span>'
    '<span class="title">Analisi Approfondita</span></div>',
    unsafe_allow_html=True,
)
st.markdown(
    """
    <div style="font-family: 'Fraunces', serif; font-style: italic; color: var(--text-dim);
        ...
        Cerca un titolo specifico per grafico e audit completo.
    </div>
    """,
    unsafe_allow_html=True,
)
